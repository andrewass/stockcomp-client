...
export const AdminUserTable = () => {
  ...
  const handlePageChange = (event: unknown, newPage: number) => {
    fetchUserEntries(newPage, rowsPerPage).catch((error) => console.log(error));
  };

  const handleChangeRowsPerPage = (event: ChangeEvent<HTMLInputElement>) => {
    fetchUserEntries(0, +event.target.value).catch((error) =>
      console.log(error),
    );
  };
        ...
        <Table>
          ...
          <TableBody>
            {userEntries.map((user) => (
              <AdminUserRow key={user.email} user={user} />
            ))}
          </TableBody>
        </Table>
      </TableContainer>

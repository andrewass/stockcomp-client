...
import {
  Participant,
  ParticipantPage,
} from "../../../domain/participant/participantTypes";
import {
...
export const ContestLeaderboard = ({ contestId }: Props) => {
  ...
  const fetchParticipantEntries = async (
    page: number,
    pageRowCount: number,
  ) => {
  ...
  const handlePageChange = (_event: unknown, newPage: number) => {
    fetchParticipantEntries(newPage, rowsPerPage).catch(console.error);
  };

  const handleChangeRowsPerPage = (event: ChangeEvent<HTMLInputElement>) => {
    fetchParticipantEntries(0, +event.target.value).catch(console.error);
  };
    ...
    <Paper>
      <TableContainer>
        <Table>
          ...
          <TableBody>
            {participantEntries!.map((participantEntry, index) => (
              <ContestLeaderboardEntry entry={participantEntry} key={index} />
            ))}
          </TableBody>
        </Table>
      </TableContainer>
